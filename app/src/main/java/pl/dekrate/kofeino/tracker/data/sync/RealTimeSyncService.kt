...
/**
 * Real-time sync service that propagates local mutations to the paired device
 * immediately via Wearable MessageAPI.
 *
 * ## How it works
 * 1. **Enqueue** — the change is written to [PendingSyncQueue] so that it
 *    survives transient failures and will be re-attempted by periodic flush.
 ...
 * This service is called from [CaffeineRepositoryImpl] which fires on local
 * mutations. Incoming changes applied via [IncomingSyncProcessor] write
 * directly to DAOs — they never go through the repository, so this service
 * is never re-triggered for inbound sync messages.
 */
@Singleton
